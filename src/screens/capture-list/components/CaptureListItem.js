...
const API_KEY = '96a3893bde7f464ba5a885dde464190f';


const CaptureListItem = ({capture}) => {
  const {latitude, longitude, image_uri, created_at} = capture;
  ...
  useEffect(() => {
    fetch(
      ...
      .then(response => response.json())
      .then(json => {
        setTemperature(_.get(json, 'data.temp', "Temperature unavailable").toString())
      })
